...
<body>
    ...
    if ($_SESSION[$ordinal][-1] == 'f') {
        ...
        $result = "与你适配的部门是：<span>策划部</span>";
    } elseif ($_SESSION[$ordinal][-1] == 'b') {
        $result = "与你适配的部门是：<span>软件部</span>";
    } elseif ($_SESSION[$ordinal][-1] == 'c') {
        $result = "与你适配的部门是：<span>美工部</span>";
    } elseif ($_SESSION[$ordinal][-1] == 'd') {
        $result = "与你适配的部门是：<span>运维部</span>";
    } elseif ($_SESSION[$ordinal][-1] == 'e') {
        $result = "与你适配的部门是：<span>运营部</span>";
    } else{
        $result = "";
    }
    ?>
    <div class="page">
        ...
        <div class="introduce">
            ...
            <div class="section">
                <p>运营</p>
                <div class="information">
                    运营部运营是实验室运作的重要一环，直接与用户群体对接。如果你喜欢写作，喜欢交流，那么就请来运营部吧。我们有高自由度的创作环境，原创文章拥有完全著作权。不干涉在其他公众号平台从事运营工作。近6万关注公众号的运营经历，如果未来你想从事新媒体工作，这将是一块不错的敲门砖。
                </div>
            </div>
        </div>
        <div class="operBar">
            <button onclick="window.location.href='enterPageControl.php'" class="oper">回首页</button>
            <button onclick="window.location.href='entryForm.php'" class="oper">去报名</button>
        </div>
    </div>
</body>
